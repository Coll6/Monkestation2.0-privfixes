...
/datum/species/ipc/proc/update_chassis(mob/living/carbon/target)
	...
	if(!features)
		return
	var/datum/sprite_accessory/ipc_chassis/chassis_of_choice = SSaccessories.ipc_chassis_list[features["ipc_chassis"]]

	if(!chassis_of_choice)
		var/random_chassis = pick(SSaccessories.ipc_chassis_list)
		chassis_of_choice = SSaccessories.ipc_chassis_list[random_chassis]
